...
import static com.jiongsoft.cocit.service.CocEntityFieldService.TYPE_FK;
import static com.jiongsoft.cocit.service.CocEntityFieldService.TYPE_NUMBER;
import static com.jiongsoft.cocit.service.CocEntityFieldService.TYPE_RICH_TEXT;
import static com.jiongsoft.cocit.service.CocEntityFieldService.TYPE_TEXT;
import static com.jiongsoft.cocit.service.CocEntityFieldService.TYPE_UPLOAD;
...
public class JCocitFormRender extends WidgetRender<EntityFormWidgetModel> {
	...
	public void render(Writer writer, EntityFormWidgetModel model) throws Throwable {
		...
		print(sb, "<input name=\"entity.id\" type=\"hidden\" value=\"%s\">", Lang.idOrtoString(formData));

		CocEntityFieldService entityField;
		String propName;
		Object objFldValue;
		/*
		 * 计算可见字段、隐藏字段、不显示的字段等
	...
	private boolean printDic(StringBuffer sb, CocEntityFieldService fieldService, Object fieldValue) {
	...
	private void printStr(StringBuffer sb, CocEntityFieldService fieldService, String fieldValue) {
	...
	private void printText(StringBuffer sb, CocEntityFieldService fieldService, Object fieldValue) {
		String str = fieldValue == null ? "" : fieldValue.toString();
		print(sb, "<textarea name=\"entity.%s\" class=\"textarea\">%s</textarea>", fieldService.getPropName(), str);
	}

	private void printDate(StringBuffer sb, CocEntityFieldService fieldService, Date fieldValue) {
		boolean isDic = this.printDic(sb, fieldService, fieldValue);
		if (!isDic) {
			String str = Lang.format(fieldValue, fieldService.getPattern());
	...
	private void printNum(StringBuffer sb, CocEntityFieldService fieldService, Number fieldValue) {
		boolean isDic = this.printDic(sb, fieldService, fieldValue);
		if (!isDic) {
			String str = Lang.format(fieldValue, fieldService.getPattern());
